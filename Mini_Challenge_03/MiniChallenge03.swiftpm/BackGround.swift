    ...
    override func update(_ currentTime: TimeInterval) {
            ...
            for (_, estrela) in self.children.enumerated().reversed() {
                if let estrelaNode = estrela as? SKSpriteNode {
                    ...
                    let screenWidth = CGFloat(self.view?.bounds.width ?? 1.0)

                    // Normalize the x-position to a value between -1 and 1
                    let normalizedX = x / (screenWidth / 2)

                    // Interpolate between red and blue based on normalized x-position
                    let redColor = max(0.0, 1.0 - normalizedX)
                    let blueColor = max(0.0, 1.0 + normalizedX)

                    estrelaNode.color = SKColor(red: redColor, green: 0.0, blue: blueColor, alpha: 0.3)
